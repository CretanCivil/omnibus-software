...
build do
   ...
   # Go get gohai
   command "#{gobin} get -d -u github.com/whpv/gohai", :env => env
   ...
   command "git checkout #{version} && git pull", :env => env, :cwd => "/var/cache/omnibus/src/datadog-gohai/src/github.com/whpv/gohai"
   command "cd $GOPATH/src/github.com/whpv/gohai && #{gobin} run make.go '#{gobin}' && mv gohai #{install_dir}/bin/gohai", :env => env
end
